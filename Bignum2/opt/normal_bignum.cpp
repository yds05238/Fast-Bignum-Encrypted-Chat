...
unsigned int normal::Bignum::bin_search(normal::Bignum &target) const noexcept {
    ...
    while (left <= right) {
        ...
        const std::pair<normal::Bignum, normal::Bignum> pnum = this->mult(mid);
...
normal::Bignum normal::Bignum::binSearch(const normal::Bignum &target) const noexcept {
    unsigned int left = 0, right = 999999999;
    while (left <= right) {
        const unsigned int mid = (left + right) / 2;
        const std::pair<const normal::Bignum, const normal::Bignum> pnum = this->mult(mid);
        if (pnum.second < target) {
            left = mid + 1;
        } else if (target < pnum.first) {
            right = mid - 1;
